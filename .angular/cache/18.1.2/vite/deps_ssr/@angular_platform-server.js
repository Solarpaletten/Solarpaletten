...
} from "./chunk-26CMSXBX.js";
import "./chunk-IC3XNFGM.js";
import {
  BrowserDomAdapter,
  BrowserModule,
...
var DominoAdapter = class _DominoAdapter extends BrowserDomAdapter {
  ...
  getBaseHref(doc) {
    var _a;
    return ((_a = doc.documentElement.querySelector("base")) == null ? void 0 : _a.getAttribute("href")) || "";
  }
...
function relativeUrlsTransformerInterceptorFn(request, next) {
  ...
  let urlPrefix = "".concat(protocol, "//").concat(hostname);
  if (port) {
    urlPrefix += ":".concat(port);
  }
...
function runAndMeasurePerf(label, method) {
  ...
  const startLabel = "start:".concat(labelName);
  const endLabel = "end:".concat(labelName);
  const end = () => {
...
function createServerPlatform(options) {
  var _a;
  const extraProviders = (_a = options.platformProviders) != null ? _a : [];
...
function removeEventDispatchScript(doc) {
  var _a;
  (_a = findEventDispatchScript(doc)) == null ? void 0 : _a.remove();
}
...
function insertEventRecordScript(appId, doc, eventTypesToReplay, nonce) {
  ...
  if (eventDispatchScript) {
    const replayScriptContents = "window.__jsaction_bootstrap(document.body," + '"'.concat(appId, '",') + "".concat(JSON.stringify(Array.from(regular)), ",") + "".concat(JSON.stringify(Array.from(capture))) + ");";
